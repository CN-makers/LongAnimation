    ...
    def __call__(self, input_image, coarse):
        ...
        if isinstance(input_image,np.ndarray):
            ...
            with torch.no_grad():
                ...
                image_list=[]
                if image.shape[0]>81:
                    image_list=torch.chunk(image, 4, dim=0)
                    # split_point = image.size(0) // 2
                    # image_list=[image[:split_point],image[split_point:]]
                elif image.shape[0]>49:
                    image_list=torch.chunk(image, 16, dim=0)
                    #image_list=[image]
                else:
                    image_list=[image]
                line_list=[]
                for image in image_list:
                    line_list.append(model(image))
                line=torch.cat(line_list,dim=0)
                #print(line.shape)
                #line = (line * 255.0).clip(0, 255).astype(np.uint8)    
                return line
